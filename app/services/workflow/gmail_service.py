...
class GmailService(metaclass=Singleton):
	def __init__(self):
	...
	def move_labels(self, message_id, from_labels, to_labels, user_id=SELF_USER):
		logger.info(f"Moving labels from {from_labels} to {to_labels} for message {message_id}")
		try:
			request_body = {
			...
			logger.error(f"An error occurred: {error.reason}")
			raise error

		logger.info(f"Labels moved successfully from {from_labels} to {to_labels} for message {message_id}")
		return message
